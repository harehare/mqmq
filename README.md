...
- **`set` on a dict costs O(dict size) per call; `has`/`keys` are comparatively cheap, but a per-key scan still adds up.** Measured by isolating construction from lookup (building an `n`-entry dict via `n` sequential `set()` calls, then measuring against the already-built dict separately): even *updating* one already-existing key in an `n`-entry dict costs roughly O(n) per `set()` call — so `n` sequential `set()` calls building up a dict cost O(n²) total — while a fixed number of `has()` calls against an `n`-entry dict grow far more slowly, closer to O(log n) than O(n). Evaluating `module name: ... end` used to find its new (namespaced) bindings via `filter(keys(inner_env), fn(nk): !has(env, nk) end)` — a `has()` call, plus a closure invocation, for every key of the *whole* post-body environment, which measured as roughly linear in environment size on its own (not the O(n²) it might look like) but with real per-key overhead on top of that baseline cost. Since `set` was confirmed to update an existing key in place and only ever append a genuinely new one at the end, `eval.mq`'s `"module"` case now takes the new keys as the tail of `keys(inner_env)` past the outer env's original length instead — no per-key scan or closure call. ~2-3x faster evaluating a `module` block with several thousand pre-existing bindings in scope, and the gap widens as the environment grows.
