...
- **`set`/`has` on a dict cost O(dict size), not O(1).** Measured directly: `n` sequential `set()` calls building up a dict take O(n²), and `n` `has()` lookups against an `n`-entry dict also take O(n²) — this host runtime's dicts don't give hash-map-style O(1) get/set/has. That makes `env` (mqmq's flat variable-binding dict) expensive to query once a program has bound many names: evaluating `module name: ... end` used to compute its new (namespaced) bindings by calling `has(env, nk)` once per key of the *whole* post-body environment, which is O(env size) *per call*, i.e. O(env size²) to evaluate one `module` block once the surrounding scope is already large. Since `set` was independently confirmed to update an existing key in place and only ever append a genuinely new one at the end, `eval.mq`'s `"module"` case now takes the new keys as the tail of `keys(inner_env)` past the outer env's original length — no per-key `has` scan needed. Up to ~3x faster evaluating a `module` block with several thousand pre-existing bindings in scope.
